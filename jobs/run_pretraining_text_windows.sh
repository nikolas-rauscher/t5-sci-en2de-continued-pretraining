#!/bin/bash
#SBATCH --job-name=pretrain_text_windows
#SBATCH --time=4:00:00
...
#SBATCH --output=logs/slurm-pretrain-text-windows-%j.out
#SBATCH --error=logs/slurm-pretrain-text-windows-%j.err
...
srun -K \
  ...
  bash -c "source .venv_pretraining/bin/activate && pip install wandb && python src/train.py \
    ...
    trainer.devices=1 \
    logger.wandb.group=text-window-comparison \
    logger.wandb.name=t5-pretraining-text-windows \
    +logger.wandb.tags='[t5-pretraining,text-windows,production,comparison]'"