#!/bin/bash
#SBATCH --job-name=smart_resume_lr001_clip1
...
#SBATCH --output=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_smart_resume_lr001_clip1.out
#SBATCH --error=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_smart_resume_lr001_clip1.err

echo "=============================================="
echo "Smart Resume T5 Continued Pretraining - LR 0.001, Gradient Clip 1.0"
...
find_latest_checkpoint() {
    ...
    RUN_DIRS=$(find pretraining_logs_lr_001_gradient_clip_1_with_inverse_sqrt_schedule/train/runs -name "20*" -type d 2>/dev/null | sort -r)
    ...
    if [[ -n "$RUN_DIRS" ]]; then
        ...
        LATEST_CKPT=$(find pretraining_logs_lr_001_gradient_clip_1_with_inverse_sqrt_schedule/train/runs -name "step-*.ckpt" 2>/dev/null | sort -V | tail -1)
        echo "$LATEST_CKPT"
    fi
}
...
echo "Configuration: configs/experiment/t5_continued_pretraining_lr_001_gradient_clip_1_with_inverse_sqrt_schedule.yaml"
...
srun -K \
    ...
    /bin/bash -c "
        ...
        python src/train.py experiment=t5_continued_pretraining_lr_001_gradient_clip_1_with_inverse_sqrt_schedule trainer.log_every_n_steps=1 $RESUME_CMD
    "
