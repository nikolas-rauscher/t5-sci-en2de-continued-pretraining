#!/bin/bash
#SBATCH --job-name=resume_t5_ada_0olap_v3_OPT
...
# Logs
#SBATCH --output=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_resume_t5_ada_0olap512_v3_OPT.out
#SBATCH --error=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_resume_t5_ada_0olap512_v3_OPT.err

echo "=============================================="
echo "SMART RESUME: T5 Adafactor OPTIMIZED - 0% Overlap"
...
RUN_DIR=$(find pretraining_logs_adafactor_relative_step_clip1_OPTIMIZED_text_0olap512_v3/train/runs -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)
...
if [ -z "$RUN_DIR" ]; then
    ...
    if [ -z "$CKPT_PATH" ]; then
        ...
        srun -K \
            ...
            /bin/bash -c "
                source .venv_pretraining/bin/activate && \
                python src/train.py experiment=t5_continued_pretraining_adafactor_relative_step_clip_1_OPTIMIZED_text_0olap512_v3 \
                    ckpt_path=\"$CKPT_PATH\" \
