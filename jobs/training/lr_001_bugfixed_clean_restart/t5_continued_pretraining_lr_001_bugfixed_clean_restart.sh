#!/bin/bash
#SBATCH --job-name=t5_lr001_bugfixed_restart
...
#SBATCH --output=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_t5_lr001_bugfixed_restart.out
#SBATCH --error=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_t5_lr001_bugfixed_restart.err

echo "=============================================="
echo "T5 Continued Pretraining - Clean Restart with Critical Bug Fix"
echo "LR: 0.001, Warmup: 60k, Gradient Clip: 1.0"
...
echo "Starting T5 continued pretraining (LR 0.001, Bug Fixed) ..."
echo "Configuration: configs/experiment/t5_continued_pretraining_lr_001_bugfixed_clean_restart.yaml"
echo "Learning Rate: 0.001 (SciFive optimized, Bug Fixed)"
...
srun -K \
    ...
    /bin/bash -c "
        ...
        python src/train.py experiment=t5_continued_pretraining_lr_001_bugfixed_clean_restart trainer.log_every_n_steps=1
    "
