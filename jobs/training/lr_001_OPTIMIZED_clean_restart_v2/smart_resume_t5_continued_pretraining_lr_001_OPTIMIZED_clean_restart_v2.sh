#!/bin/bash
# Smart resume script for LR=0.001 OPTIMIZED clean-restart v2 (inverse-sqrt)

#SBATCH --job-name=smart_resume_lr001_OPTIMIZED_v2
...
#SBATCH --output=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_smart_resume_lr001_OPTIMIZED_v2.out
#SBATCH --error=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_smart_resume_lr001_OPTIMIZED_v2.err

echo "=============================================="
echo "Smart Resume T5 Continued Pretraining - OPTIMIZED COLLATOR v2"
echo "LR 0.001, Clip 1.0, Clean Restart v2"
...
find_latest_checkpoint() {
    ROOT_DIR="pretraining_logs_lr_001_OPTIMIZED_clean_restart_v2/train/runs"
...
echo "Looking for latest OPTIMIZED v2 checkpoint..."
LATEST_CKPT=$(find_latest_checkpoint)

if [[ -z "$LATEST_CKPT" ]]; then
    echo "No checkpoint found - starting fresh OPTIMIZED v2 training"
    RESUME_CMD=""
else
...
echo "Starting T5 OPTIMIZED v2 training with smart resume..."
echo "Configuration: configs/experiment/t5_continued_pretraining_lr_001_OPTIMIZED_clean_restart_v2.yaml"
...
srun -K \
    ...
    /bin/bash -c "
        ...
        python src/train.py experiment=t5_continued_pretraining_lr_001_OPTIMIZED_clean_restart_v2 trainer.log_every_n_steps=1 $RESUME_CMD
    "
