#!/bin/bash
# Smart resume script for LR=0.001 bugfixed clean-restart (inverse-sqrt)

#SBATCH --job-name=smart_resume_lr001_bugfix
...
#SBATCH --output=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_smart_resume_lr001_bugfix.out
#SBATCH --error=/netscratch/nrauscher/projects/BA-hydra/logs/slurm_%j_smart_resume_lr001_bugfix.err

echo "=============================================="
echo "Smart Resume T5 Continued Pretraining - LR 0.001, Clip 1.0 (Bugfixed Clean Restart)"
...
find_latest_checkpoint() {
    ROOT_DIR="pretraining_logs_lr_001_bugfixed_clean_restart/train/runs"
...
echo "Configuration: configs/experiment/t5_continued_pretraining_lr_001_bugfixed_clean_restart.yaml"
...
srun -K \
    ...
    /bin/bash -c "
        ...
        python src/train.py experiment=t5_continued_pretraining_lr_001_bugfixed_clean_restart trainer.log_every_n_steps=1 $RESUME_CMD
    "
